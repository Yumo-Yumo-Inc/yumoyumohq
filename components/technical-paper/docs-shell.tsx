...
type NavSection = {
  parent: TechnicalPaperNavItem;
  children: TechnicalPaperNavItem[];
};

function groupNavigation(items: TechnicalPaperNavItem[]): NavSection[] {
...
type DocsShellProps = {
  locale: TechnicalPaperLocale;
  currentPath: string;
  navigation: TechnicalPaperNavItem[];
  toc: TechnicalPaperTocItem[];
  previous: TechnicalPaperNavItem | null;
  next: TechnicalPaperNavItem | null;
  languageLinks: LanguageLink[];
  children: React.ReactNode;
};

export function TechnicalPaperDocsShell({
  locale,
  currentPath,
  navigation,
  toc,
  ...
  const ui = technicalPaperUiStrings[locale];

  const sections = useMemo(() => groupNavigation(navigation), [navigation]);

  // Track which sections are expanded. By default, only the section whose
  // page the viewer is currently on is open; the others stay collapsed so
  // the sidebar reads as a clean list of top-level chapters.
  ...
  return (
    <div
      ...
      style={{ background: BG_BASE, color: TEXT_PRIMARY }}
    >
      {/* Sub-header — slim breadcrumb strip below the site Navigation. */}
      <header
      ...
      >
        <div className="mx-auto flex w-full max-w-[1400px] items-center gap-3 px-4 py-2.5 md:px-6">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <Code2 className="h-3.5 w-3.5 shrink-0" style={{ color: ACCENT }} />
            <span
        ...
        >
          <div className="sticky top-[calc(4rem+57px)] h-[calc(100vh-4rem-57px)] overflow-y-auto px-5 py-6">
            <div className="mb-5">
              ...
              >
                {technicalPaperLocaleLabels[locale]}
              </div>
              <div
                  ...
                  <div key={parent.href}>
                    ...
                    {parentOnPage && toc.length > 0 ? (
                      <ul
                        ...
                        style={{
                          borderLeft: `0.5px solid ${BORDER_STRONG}`,
                        }}
                      >
                            ...
                            <div key={child.href}>
                              ...
                              >
                                <span
                                  ...
                                  style={{
                                    background: childActive ? ACCENT : TEXT_MUTED,
                                  }}
                                />
                                <span className="block min-w-0 flex-1">
                                  {child.title}
        ...
        {drawerOpen ? (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div
              ...
              style={{
                background: BG_ELEVATED,
                borderRight: `0.5px solid ${BORDER_STRONG}`,
              }}
            >
                    ...
                    <div key={parent.href}>
                      ...
                      >
                        <Link
                          ...
                          style={{
                            color: parentOnPage || sectionActive
                              ? TEXT_PRIMARY
                              : TEXT_SECONDARY,
                            fontFamily: MONO_STACK,
                            paddingLeft: "10px",

...
import { AcademiaSourcesTable } from "@/components/academia/sources-table";
...
// Minimal JSON syntax highlighter — no Shiki / Prism dependency.
function highlightJson(source: string): React.ReactNode[] {
  ...
  while (i < source.length) {
    ...
    if (source[i] === '"') {
      ...
      out.push(
        <span key={key++} style={{ color: isKey ? "#93C5FD" : "#FCA5A5" }}>
          {slice}
        </span>
      );
      i = j + 1;
...
type AcademiaMarkdownProps = {
  markdown: string;
  locale: AcademiaLocale;
};

export function AcademiaMarkdown({ markdown, locale }: AcademiaMarkdownProps) {
  return (
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          thead: ({ children }) => (
            <thead
              style={{
                ...
                borderBottom: "0.5px solid rgba(167, 139, 250, 0.3)",
              }}
            >
              {children}
            </thead>
          ...
          pre({ children }) {
            ...
            // Live source registry — pulled from the DB at render.
            if (language === "academia-sources") {
              return <AcademiaSourcesTable locale={locale} />;
            }

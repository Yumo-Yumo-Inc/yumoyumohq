...
Yumo Yumo 的经济骨架在日常使用与开放协调之间架起多层桥梁。支出凭证、商户验证、商品改进以及社区任务先沉淀到 bINT 层。这个层级让贡献的质量、信任与持续性变得可见。INT 层承载更广泛的经济协调、质押以及随时间成熟的治理界面。Foundation NFT — Yumbie — 在系统内锚定用户可携带的数字身份，而每张凭证都会作为分析结果向用户呈现隐性成本洞察。

这种分层很重要，因为贡献、价值与身份各自走不同的门。为系统创造价值的用户先积累 bINT。时间、持有行为与信任决定这些余额如何走向 INT。每张已验证票据都会呈现隐性成本洞察，向用户显示隐藏在其支出中的压力。用户的 Yumbie 承载这段旅程的可见记忆。结果是一种奖励稳定且可信参与、并使价值与长期贡献保持一致的经济。
...
| **bINT** | 链下记账单位（运营层） | 不可 — 通过既定生命周期结算为 INT | 贡献记账；工作与奖励之间的柔性层 |
| **Foundation NFT（Yumbie）** | Token-2022 NonTransferable | 不可 | 持久的数字身份；与用户共同进化的视觉伙伴 |

bINT 是从每张已验证票据中产生的贡献信号。它衡量 Yumo Yumo 经济内部的贡献强度，并通过与时间、持有行为与信任相关的既定逻辑进行转换。
...
在代币生成事件之前，平台通过 cPoints 跟踪贡献，这是一种封闭系统的声誉度量。cPoints 仅存在于 TGE 前阶段。它们为 TGE 时的初始空投与入驻权重提供信息，之后即被弃用。从 TGE 开始，bINT 层以更强的贡献语义取代 cPoints 的角色；bINT 是一种离链记账单位，通过既定的生命周期结算为 INT。
...
flowchart LR
    ...
    B -->|按年比率| D[INT 转换]
    D --> E[质押 / 持有 / 转账]
    C -->|第 30 级| F[Smart Agent 进化]
    E --> G[金库回购与销毁]
```

每张已验证票据同时向用户呈现隐性成本洞察、把贡献写入 bINT、把身份进展写入用户的 Yumbie。bINT 到 INT 的转换按有利于早期参与且随时间趋于平衡的比例，把价值从贡献层移到经济层。质押把价值回馈给长期持有者。由金库管理的回购与销毁通过把真实平台收入与代币稀缺性挂钩来闭合循环。

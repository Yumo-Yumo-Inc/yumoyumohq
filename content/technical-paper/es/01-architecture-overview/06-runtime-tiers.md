...
|---|---|---|---|---|
| Sincrónico (solicitud) | LLM de visión (OCR para PDFs) → validación regex → resolución de comerciante → cálculo de recompensa | P95 < ~5 s de extremo a extremo | < 0,02 $ / recibo | Mostrar vista previa degradada + reintentar |
| Asincrónico (liquidación) | Resolución de producto canónico, actualización del puntaje de confianza, raíz de distribución por época, liquidación de reclamaciones INT, actualizaciones de nivel NFT | < 1 hora desde la verificación | Tarifas de Solana | Diferir al siguiente lote |

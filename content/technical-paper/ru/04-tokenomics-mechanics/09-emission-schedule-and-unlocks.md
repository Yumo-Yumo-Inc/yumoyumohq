...
Рельс User Rewards (64,35 млрд INT) выпускается на 15-летнем горизонте. Ежедневная эмиссия дозируется ступенчатой функцией, масштабируемой по ежемесячным активным пользователям (MAU).
...
|---|---|---|
| **User Rewards (65%)** | Эмиссионная кривая (4.19) → начисление bINT вне блокчейна → периодический epoch-расчёт → получение из дистрибьютора (4.4) | Непрерывно в течение 15 лет |
...
- **Airdrop** — выпускается в течение нескольких периодов как кампанийные распределения, а не одним событием. Для каждого распределения набор получателей и метод расчёта фиксируются в блокчейне до движения токенов. Каждая доля забирается без вестинга через выделенный дистрибьютор, отдельный от периодического расчёта пользовательских вознаграждений. Объём распределения публикуется в записи соответствующей кампании.
...
- **График выпуска стейкингового пула.** Дата активации, график выпуска и связанные условия публикуются отдельно.

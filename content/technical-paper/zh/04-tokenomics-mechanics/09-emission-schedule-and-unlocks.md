...
| 轨道 | 解锁机制 | 时间安排 |
|---|---|---|
| **用户奖励（65%）** | 发行曲线（4.19）→ 链下 bINT 累积 → 周期性 epoch 结算 → 从分发器领取（4.4） | 15 年内持续进行 |
...
- **空投** — 作为活动分发，分多个周期释放，而非单一事件。每次分发的接收者集合与计算方法在代币转移之前承诺在链上。每份额度无归属期，通过一个独立于周期性用户奖励结算的专用分发器领取。分发规模在相应的活动记录中公布。
...
剩余约 98.99% 的供应量锁定于发行时间表、归属合约、质押池、受治理的储备以及多周期空投计划中。空投分发作为活动分发逐步进入流通，而非在 TGE 时进入。该初始流通量遵循上述分配与释放时间表。
